...
def clean_dict(dictionary) :
    print "Keys: ", dictionary.viewkeys()
    print "Key-value pair: ", dictionary.items()
    for key in dictionary.viewkeys() :
...
def chk_ans(first, answer):
    ...
    if answer == first:
        print "You're right!"
         ...
         print "Sorry - correct answer is ", first

chk_ans(FIRST_NAME, ANSWER)

def get_fname() :
    for key in PLAYER_BY_NAME :
        FIRST_NAME.append(key)
        LAST_NAME.append(PLAYER_BY_NAME.get(key))
    print "First names: ", FIRST_NAME
    print "Last names: ", LAST_NAME
    random_lname = random.choice(LAST_NAME)
    expected_answer = FIRST_NAME[LAST_NAME.index(random_lname)]
    ANSWER = raw_input("What is the first name of " + random_lname + " ")
    chk_ans(expected_answer.lower(), ANSWER.strip().lower())
...
def check_car_list(brand, answer) :
    found = False
    for values in cars.get(brand) :
        if values.lower() == answer.lower() :
            found = True
    return found        
            
brand = random_key(cars)
answer = raw_input("Name a car for " + brand + " ")
if check_car_list(brand, answer) :
